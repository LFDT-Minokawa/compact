 ...
 */
export function createZswapInput(circuitContext: CircuitContext<unknown>, qualifiedCoinInfo: EncodedQualifiedCoinInfo): void {
 ...
 */
export const hasCoinCommitment = (context: CircuitContext<unknown>, coinInfo: EncodedCoinInfo, recipient: EncodedRecipient): boolean =>
  context.transactionContext.comIndicies.has(Buffer.from(Bytes32Descriptor.fromValue(createCoinCommitment(coinInfo, recipient).value)).toString('hex'));
