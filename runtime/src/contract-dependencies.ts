...
import { AlignedValue, ContractAddress, StateMap, StateValue } from '@midnight-ntwrk/onchain-runtime';
import { CompactError, CompactType } from './runtime';
 ...
 */
type CompactValue = ContractAddress | CompactVector | CompactStruct;
...
function isCompactValue(x: unknown): x is CompactValue {
  return isContractAddress(x) || isCompactVector(x) || isCompactStruct(x);
}
 ...
 */
function assertIsContractAddress(value: CompactValue): asserts value is ContractAddress {
...
const compactValueDependencies = (
  ...
  dependencies: Set<ContractAddress>,
): void => {
 ...
 */
const alignedValueToCompactValue = (descriptor: CompactType<unknown>, { value }: AlignedValue): CompactValue =>
 ...
 */
const stateValueToCompactValue = (descriptor: CompactType<unknown>, stateValue: StateValue): CompactValue =>
 ...
 */
function assertCastSucceeded<S extends StateMap | StateValue[]>(
  s: S | undefined,
 ...
 */
const castToStateArray = (state: StateValue): StateValue[] => {
...
export const contractDependencies = (
  ...
  const dependencies = new Set<ContractAddress>();
