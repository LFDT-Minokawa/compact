...
import type { EncodedZswapLocalState } from './zswap';
import { freshZswapLocalStates } from './zswap';
import type { PartialProofData, ProofData } from './proof-data';
...
const createQueryContexts = (
  ...
  Object.fromEntries(
    Object.entries(contractStates).map(([address, contractState]) => [
      address,
      new ocrt.QueryContext(contractState instanceof ocrt.ContractState ? contractState.data : contractState, address)
    ]),
  );
 ...
 * @param circuitContext The context for the current circuit.
 */
export const saveCurrentPrivateState = ({
                                          privateStates,
                                          contractAddress,
                                          currentPrivateState,
                                        }: CircuitContext): void => {
...
export const queryLedgerState = (
  ...
  try {
    ...
    if (err instanceof Error) {
      throw new CompactError(err.message);
    }
    throw err;
  }
};
