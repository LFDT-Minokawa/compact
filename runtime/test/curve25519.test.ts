...
describe('curve25519 point validation', () => {
  ...
  test('accepts the generator and computed points', () => {
    expect(runtime.curve25519IsValidPoint(G)).toBe(true);
    expect(runtime.curve25519IsValidPoint(runtime.curve25519MulGenerator(7n))).toBe(true);
    expect(runtime.curve25519IsValidPoint(negate(G))).toBe(true);
  });

  test('the identity is accepted, it lies on the curve', () => {
    expect(runtime.curve25519IsValidPoint(IDENTITY)).toBe(true);
  ...
  test('rejects a point that is not on the curve', () => {
    ...
    expect(runtime.curve25519IsValidPoint({ x: 1n, y: 1n })).toBe(false);
    expect(runtime.curve25519IsValidPoint({ x: G.x, y: G.y + 1n })).toBe(false);
  });

  test('rejects (0, 0), which is the identity only on the secp curves', () => {
    expect(runtime.curve25519IsValidPoint({ x: 0n, y: 0n })).toBe(false);
  });

  test('rejects a point from another curve', () => {
    const alien = runtime.secp256k1MulGenerator(1n);
    expect(runtime.curve25519IsValidPoint({ x: alien.x % P, y: alien.y % P })).toBe(false);
  });

  test('rejects a coordinate that is not reduced', () => {
    expect(runtime.curve25519IsValidPoint({ x: P, y: G.y })).toBe(false);
    expect(runtime.curve25519IsValidPoint({ x: G.x, y: P + G.y })).toBe(false);
    // (p, 1) reduces to the identity, but is still not a valid encoding of it.
    expect(runtime.curve25519IsValidPoint({ x: P, y: 1n })).toBe(false);
  });

  test('rejects a negative coordinate', () => {
    expect(runtime.curve25519IsValidPoint({ x: -G.x, y: G.y })).toBe(false);
    expect(runtime.curve25519IsValidPoint({ x: G.x, y: -1n })).toBe(false);
  });
  ...
  test('accepts a small-order point, only the curve equation is checked', () => {
    ...
    expect(runtime.curve25519IsValidPoint(order2)).toBe(true);
