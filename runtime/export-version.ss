...
(let ([out-ts (open-output-file "src/version.ts" 'replace)])
  ...
  (fprintf out-ts
    "~{\n~A~}\n"
    `(
     "export const checkRuntimeVersion = (expectedMaxField: bigint, expectedRuntimeVersionString: string): void => {"
     ...
     "  }"
     "  if (expectedMaxField !== OCRT_MAX_FIELD) {"
     "    throw new CompactError(`Maximum field mismatch: compiled code uses ${expectedMaxField}, runtime uses ${OCRT_MAX_FIELD}`);"
     "  }"
     "}"))
  (close-port out-ts))
