...
const renderReport = (
    ...
    lines.push('');
    lines.push(
        'Every name the Compact language offers, and where a `test-contracts` fixture ' +
            'uses it. The names are read from the compiler, so anything added there ' +
            'appears here on the next run — whether or not a fixture exists for it.',
    );
    lines.push('');
    lines.push(
        "This is the counterpart to `./compiler/go`, which measures the compiler's own " +
            'code. That number cannot distinguish a covered standard-library circuit ' +
            'from an uncovered one, because they compile through identical code.',
    );
    lines.push('');
    lines.push(
        `Scanned the **${fixtures.length} fixtures** in \`test-contracts/\`. Nothing ` +
            'else. A name listed as uncovered may still be exercised by the e2e suite, ' +
            'which compiles contracts out of `examples/` — so read this as a to-do list ' +
            'for this suite, not as a claim about the repository.',
    );
    lines.push('');
