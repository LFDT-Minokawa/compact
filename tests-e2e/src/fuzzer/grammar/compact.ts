 ...
 * spells Compact syntax: shapes.cjs is list combinators with no Compact in them,
 * tags.cjs is the validity tags, index.cjs assembles and checks, utils/ generates
 * the terminals. The grammar used to be spread over fifteen modules plus the
 * helpers and the entry-point map, and it drifted.
 *
 *   TERMINALS      produced by utils/fuzzer.cjs; declared here so they can be checked
...
import { join, repeat } from './shapes';

/* ================================================================== *
 * Terminals and entry points
 * ================================================================== */

/* Generated by utils/fuzzer.cjs rather than by a production here. */
export const TERMINALS: readonly Token[] = [
...
const call = (name: Token, genericNodes: Token[], argNode: Token, arity: number): Alternative =>
    [name, ...generics(genericNodes), '(', ...repeat(argNode, arity, ', '), ')'];

const method = (receiver: Token, op: Token, argNode: Token, arity: number): Alternative =>
    ['optional_statement_variable', receiver + op + '(', ...repeat(argNode, arity, ', '), ')', 'valid_end_line'];
...
/* The value shapes a generated `const` can bind. */
const BINDING_VALUES: Record<string, (name: Token, typeNode: Token) => Alternative> = {
    default: (n, t) => ['const ', n, ' = ', 'default<', t, '>', 'end_line'],
    typedDefault: (n, t) => ['const ', n, ' : ', t, ' = ', 'default<', t, '>', 'end_line'],
    pad: (n) => ['const ', n, ' = ', 'pad(', 'random_number', ', "', 'random_string', '")', 'end_line'],
    smallNumber: (n) => ['const ', n, ' = ', 'small_random_number', 'end_line'],
    number: (n) => ['const ', n, ' = ', 'random_number', 'end_line'],
    string: (n) => ['const ', n, ' = ', 'random_string', 'end_line'],
    sliceOfDefault: (n, t) => ['const ', n, ' = ', 'slice<', 'random_number', '>(default<', t, '>, ', 'random_number', ')', 'end_line'],
    sliceOfTuple: (n) => ['const ', n, ' = ', 'slice<', 'random_number', '>([', 'random_mixed_table', '], ', 'random_number', ')', 'end_line'],
    // slice() takes a value; a type there cannot parse
    sliceOfType: (n, t) => ['const ', n, ' = ', 'slice<', 'random_number', '>(', t, ', ', 'random_number', ')', 'end_line'],
    spreadSliceOfType: (n, t) => ['const ', n, ' = ', '[...slice<', 'random_number', '>(', t, ', ', 'random_number', ')]', 'end_line'],
};

const bindings = (name: Token, kinds: string[], typeNode: Token = 'valid_types'): Alternative[] =>
    kinds.map((kind) => BINDING_VALUES[kind](name, typeNode));
...
const harness: Grammar = {
    ...
    assert_body: [['{\n ', 'assert_variable_a', 'assert_variable_b', 'assert_statement', '\n}']],
    if_statements: [contract('constructor_harness', 'if_body')],
    if_body: [
        ['{\n ', 'if_variable_a', 'if_variable_b', 'if_variable_c', 'if_statement', '\n}'],
        // ['{\n ', 'generate_nested_if', '\n}'],
    ],
    for_statements: [
    ...
    circuit_body: [
        ['{\n ', 'body_assert_statement', 'circuit_return_statements', '\n}'], 
...
const declarations: Grammar = {
    ...
    import_library: identifierPosition([
    ...
    prefix_string: identifierPosition([
        ['CompactStandardLibrary'],
        ['"CompactStandardLibrary"'],
        // `valid_types` was written `valid_type`, undefined, and emitted as literal text
        ['valid_types'],
...
const statements: Grammar = {
    constructor_assert_statements: assertStatement,
    constructor_return_statements: returnStatements('constructor_return_statements'),
    // circuit and constructor bodies open with the same assertion
    body_assert_statement: assertStatement,
    ...
    // one binding shape, two names
    assert_variable_a: bindings('bob', ASSERT_BINDINGS),
    assert_variable_b: bindings('tom', ASSERT_BINDINGS),
    assert_statement: [
    ...
    // one binding shape, three names; `greg` also gets the tuple form
    if_variable_a: bindings('bob', IF_BINDINGS),
    if_variable_b: bindings('tom', IF_BINDINGS),
    if_variable_c: bindings('greg', [...IF_BINDINGS, 'sliceOfTuple']),
    if_statement: [
    ...
    // four names, one binding shape
    variable_a: bindings('bob', STATEMENT_BINDINGS, STATEMENT_TYPE),
    variable_b: bindings('tom', STATEMENT_BINDINGS, STATEMENT_TYPE),
    variable_c: bindings('greg', STATEMENT_BINDINGS, STATEMENT_TYPE),
    variable_d: bindings('adonis', STATEMENT_BINDINGS, STATEMENT_TYPE),
...
const ledgerAdts: Grammar = {
    single_statements: [
        ...Object.keys(LEDGER_ADT_TYPES).map((adt) =>
            contract(`single_${adt}`, 'statement_declaration', `single_${adt}_body`)),
        contract('statement_declaration', 'single_kernel_body'),
    ],
    single_kernel_body: [['{\n ', 'no_variable_kernel', 'no_variable_after_statement', '\n}']],

    ...Object.fromEntries(Object.entries(LEDGER_ADT_TYPES).flatMap(([adt, type]) => [
        [`single_${adt}`, [['export ledger var_' + adt + ': ', ...type, 'valid_end_line']]],
    ...
    ...Object.fromEntries(Object.entries(LEDGER_OPS).flatMap(([adt, ops]) => {
        ...
        return [
            [adt, [[`variable_${adt}`], [`no_variable_${adt}`]]],
            [`variable_${adt}`, ops.flatMap((op) => [
                ['optional_statement_variable', recv + op + '(bob, tom)', 'valid_end_line'],
                method(recv, op, 'statement_variable', 1),
                method(recv, op, 'statement_variable', 2),
            ])],
            [`no_variable_${adt}`, [
                // an operation this ADT does not have
                ['optional_statement_variable', recv, 'random_input', '()', 'valid_end_line'],
                ...ops.flatMap((op) => [method(recv, op, 'random_input', 0), method(recv, op, 'random_input', 1)]),
            ]],
        ];
    })),
};
...
const stdlib: Grammar = {
    ...
    variable_statement_methods: STDLIB_CALLS.flatMap((c) => [
        call(c.name, c.generics, 'statement_variable', c.args),
        call(c.name, c.generics, 'statement_methods', c.args),
    ]),
    no_variable_statement_methods: STDLIB_CALLS.flatMap((c) => [
        call(c.name, c.generics, 'random_input', 0),
        call(c.name, c.generics, 'random_input', c.args),
        call(c.name, c.generics, 'no_variable_statement_methods', c.args),
    ]),
};
