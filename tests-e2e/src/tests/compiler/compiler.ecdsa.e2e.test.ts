...
// The four exported circuits of examples/ecdsa/example.compact, as plain pure functions.
...
describe('[ECDSA] examples/ecdsa/example_one.compact', () => {
    ...
    function sign(digest: Uint8Array): SignatureWithRecovery {
        const sig = secp256k1.Signature.fromBytes(secp256k1.sign(digest, sk, { format: 'recovered' }), 'recovered');
    ...
    test.fails('proveEthereumSignature accepts a valid signature [WIP gates]', () => {
    ...
    test.fails('proveBitcoinSignature accepts a valid signature [WIP gates]', () => {
    ...
    test.fails('recoverEthereumPublicKey recovers the signing public key [WIP gates]', () => {
