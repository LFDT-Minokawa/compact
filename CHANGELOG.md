...
- `contract-manifest.json` includes the hash for all files generated by compactc.
  Previously, the behavior relied on the relative paths against the process's
  current working directory, instead of getting the exact path of the output
  directory. So when a file wasn't found, it was excluded from `contract-manifest.json`
  despite the fact that it was generated by the compiler.
