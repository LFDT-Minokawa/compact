...
the corresponding contract type definition.  A contract value (i.e., a
value with a contract type) might be represented by the address of a
deployed contract.
...
can satisfy a contract type, disqualifying any that call witnesses.
It also imposes limitations on how the implementing code artifacts are
found and loaded.
See [Implementation-specific limits](#implementation-specific-limits) for details.
...
**Cross-contract calls:** Circuits named in contract types are called
through references to contract values, using the form

<div className="lang-ref-table">
| [*expr<sub>8</sub>*](#expression8) `.` [*id*](#identifier) `(` [*expr*](#expression) `,` ⋯ `,` [*expr*](#expression) `,`<sup>opt</sup> `)` |
</div>

where *expr<sub>8</sub>* is an expression with a contract type and
*id* is the name of one of the circuits declared in that type.

**No generic parameters for anonymous circuits and cross-contract calls:**
...
Cross-contract calls also cannot have generic parameters, because the
...
* **For a cross-contract call:** The type of the expression before the
  dot (i.e., period or full stop) must be a contract type *T*.  The
  identifier following the dot must be the name of one of the circuits
  declared in *T*.  Type checking the call then proceeds as in the
  preceding **named function** case, using the signature associated
  with the identifier in *T*.
  
  Because overloading and generic parameters are not present in
  contract types, type checking the cross-contract call reduces to
  verifying that the circuit named in the call is one of the circuits
  named in *T* and verifying that the named circuit is compatible with
  number and types of the argument expressions.  It is a static error
  if they are not compatible.
  
  The static type of a cross-contract call is the declared return type
  of the called circuit in the contract type.
...
- When calling a circuit in another contract (i.e., making a
  ...
  might make its arguments publicly visible.
...
**Limitations on contract types and cross-contract calls:**
...
access to private state is through its arguments, and its only means
of affecting private state is through the values it returns.
...
than the code provided by the calling application.  What this
generally means is that an application satisfies a contract type `T`
by compiling a contract defined in `T.compact`, and the resulting
compilation artifacts are made available alongside those of any
callers of `T`'s circuits.

The real dependency, however, is not on the source file, but instead
on the generated JavaScript file.  The code that is generated at the
site of a call to one of `T`'s circuits expects to find the
implementation of those circuits in the JavaScript file
