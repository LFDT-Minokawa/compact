...
Each circuit declaration must be "simple"; that is, it must not make
use of generic parameters.  The declaration may require the circuit to
be pure.
...
value with a contract type) might be represented by the address of the
deployed contract.
...
program by calling a circuit or constructor with the address of a
deployed contract or by returning the address of a deployed contract
...
To make a cross-contract call to a circuit in another contract, *fun*
can be of the form *e*`.`*id*, where *e* is an expression with a
contract type and *id* is the name of one of the circuits declared in
that type.
...
* **For a cross-contract call:** The type of the expression before the
  ...
  preceding **named function** case, but without the possibility of
...
witnesses.  A called contract may define witness functions, but
those functions cannot be called directly or indirectly by the
exported circuits that satisfy a contract type.  This implies that
every cross-contract call is to a circuit whose only access to private
state is through its arguments, and its only means of affecting
private state is through the values it returns.

Furthermore, In order to satisfy a contract type `T`, a contract must
be defined in the file `T.compact`, and its compilation artifacts must
...
finding compiler outputs in the code it generates, then the outputs of
compiling `T.compact` must be able to be found on that search path
when executing any circuits that make cross-contract calls using
references of type `T`.  This effectively limits any DApp to a single
implementation of each contract type.
