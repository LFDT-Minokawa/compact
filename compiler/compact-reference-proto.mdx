...
Compact datatypes.
The final subsection describes how the DApp can interact with the serialized
on-chain representations of Compact datatypes.
...
Values to be stored in the public state require size, length, and other
...
For this purpose, the `@midnight/compact-runtime` package
(described in [Compact runtime API](/api-reference/compact-runtime))
provides explicit run-time types satisfying the `CompactType<`*T*`>` interface,
where *T* is the corresponding TypeScript type.
This representation is *not* relevant to most DApp developers; knowledge of it is
useful only for developers that choose to replicate, augment, or modify the behavior
of the operations provided by `@midnight/compact-runtime`.
...
Type aliases are represented in the same manner as their underlying types.
