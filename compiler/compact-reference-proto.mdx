...
be impure if the body of the circuit contains a ledger operation, a log operation,
...
A log has the form `log(`*e*`)` where *eE is an expression.
*e* must have an event type. Event types are a pre-defined set of struct types.
[Compact standard library](compact-standard-library) provides detailed descriptions of supported events.

The type of evey `log` form is `[]`.

An attempt to `log` a non-event expression is a static error.
An attempt to `log` an event in the construction is a static error.

Evaluation of `log(`*e*`)` proceeds by evaluating *e*, computing its canonical
...
The byte encoding is done by instantiating the `serialize<`*T*, *#n*`>` for
the event to be logged. The generic serialize and deserialize circuits are
defined in Compact standard library. The compact compiler serializes the argument to
`log` by default, however, it does not provide the deserialize circuit by default.
Thus, if a consumer requires the deserialize they must define an instance of it in
their Compact program.

For example, the following circuit emits a `ShieldedSpend` event each time
it is invoked and if the consumer wants to decode the logged `nullifier`
they most include `deserializeShieldedSpend`:
