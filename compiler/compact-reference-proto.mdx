...
An emit has the form `emit(`*e*`)` where *e* is an expression.
...
The type of evey `emit` form is `[]`.

An attempt to `emit` a non-event expression is a static error.
An attempt to `emit` an event in the construction is a static error.
...
the event to be emitted. The generic serialize and deserialize circuits are
defined in Compact standard library. The compact compiler serializes the argument to
`emit` by default, however, it does not provide the deserialize circuit by default.
...
it is invoked and if the consumer wants to decode the emitted `nullifier`
