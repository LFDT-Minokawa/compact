...
Reviewed 25 September 2026 for compatibility with Compact language version 0.26.105.
...
name things.
Syntactically, an *identifier* is a *token* (atomic sequence of characters),
beginning with a character in the Unicode `ID_Start` category, a dollar sign
(`$`), or an underscore (`_`), followed by zero or more characters in the
Unicode `ID_Continue` category, dollar signs, or underscores.
`ID_Start` covers the letter categories (`Lu`, `Ll`, `Lt`, `Lm`, `Lo`, `Nl`);
`ID_Continue` additionally covers digits (`Nd`), combining marks (`Mn`, `Mc`)
and connector punctuation (`Pc`).
...
reserved for future use in Compact, e.g., `class` and `function`.
...
evaluates to `42`.
The second (inner) binding of `answer` to `12` is visible throughout the inner
...
- A [contract definition](#contract-types) defines a contract type.
- An [implements declaration](#contract-types) declares that a contract satisfies a contract type.
...
The standard library's *native* types and circuits, such as `JubjubPoint` and
`ecAdd`, cannot be exported from a contract's top level; it is a static error
to try.
A program can instead export a type alias for a native type, and export a
circuit that wraps a native circuit.
...
- `JubjubScalar` and the foreign field types (`Secp256k1Base`,
  `Secp256k1Scalar`, `Secp256r1Base`, `Secp256r1Scalar`, `Curve25519Base`,
  `Curve25519Scalar`): `0`
- Curve point types (`JubjubPoint`, `Secp256k1Point`, `Secp256r1Point`,
  `Curve25519Point`): the additive identity point.
...
Compact provides no constructor for the foreign curve point types,
so `default<`*T*`>` is the only way to create one in circuit.
...
- **`Opaque<"Uint8Array">`:**
  ...
  strict equality (`===`) operator.
- **Field types other than `Field`**, i.e. `JubjubScalar` and the foreign field
  types: the values are equal if the underlying field elements are equal.
- **Curve point types**, i.e. `JubjubPoint` and the foreign curve point types:
  an additive identity point is equal only to another additive identity point;
  otherwise, the values are equal if their coordinates are equal.
  Other relational comparisons are not supported for these types.
...
type, that is, a `Field`, a `Uint`, one of the foreign field types
(`Secp256k1Base`, `Secp256k1Scalar`, `Secp256r1Base`, `Secp256r1Scalar`,
`Curve25519Base`, `Curve25519Scalar`), or a type alias for one of these.
When both operands have the same foreign field type, the operation is performed
modulo that field's modulus and the result has that type; the rules below cover
the `Field` and `Uint` cases.
Arithmetic is not supported for `JubjubScalar`.
...
export circuit setUpDoubleAdd(a: Adder, n: Uint<64>): [] {
  disclose(a).setAddAmount(disclose((n + n) as Uint<64>));
}
```

Two details in this example are worth noting.
The cast is needed because `n + n` has type `Uint<0..`2<sup>65</sup>`>`, which
is not a subtype of the declared parameter type `Uint<64>`.
The `disclose` wrappers are needed because `setAddAmount` is not declared
`pure`, so both the contract reference and the argument cross a disclosure
boundary; see [explicit disclosure](#explicit-disclosure).
...
Cross-contract calls resolve the callee's implementation at run time via an
application-supplied *contract module provider* whose `resolve(address)` returns
the module deployed at that address, or nothing when the application has no
binding for it.

Because resolution happens at run time, an application can use different
implementations of the same contract type at different addresses, and deploying
a new implementation at an address does not require recompiling its callers.

Before entering the callee, the runtime checks the resolved module against the
caller's contract type and against the chain.
The conformance check compares the circuit signatures the contract type declares
against those the module provides, covering existence, purity, provability, arity,
argument types and result types; a module that does not implement the contract
type is reported as such.
Key agreement follows, checking the called circuit's verifier key and every other
circuit the two sides have in common.
...
the call fails at run time: a re-entrancy guard in the runtime detects both direct
re-entry (`A` calling `A`) and indirect re-entry (`A` calling `B` calling `A`)
and throws an error.
Thus, contracts cannot implement either direct or indirect recursion using
calls through references to contract values.
This restriction reflects a limitation in the current Midnight ledger and might
be lifted if the limitation is removed.
...
#### Casts to and from the other field types

Besides `Field`, Compact has the embedded-curve scalar type `JubjubScalar` and
the foreign field types `Secp256k1Base`, `Secp256k1Scalar`, `Secp256r1Base`,
`Secp256r1Scalar`, `Curve25519Base` and `Curve25519Scalar`.

- Any cast from `Field` or from a `Uint` type to `JubjubScalar` is allowed and
  cannot cause a dynamic error: the value is reduced modulo the Jubjub scalar
  modulus.  Round-tripping a `Field` through `JubjubScalar` can therefore give
  back a different value.
- Any cast from `JubjubScalar` to `Field` is allowed and cannot cause a dynamic
  error, because the maximum `JubjubScalar` value is less than the maximum
  `Field` value.  A cast from `JubjubScalar` to a `Uint` type is allowed but
  causes a dynamic error if the value is too large for the target type.
- Any cast from one of the foreign field types to `Bytes<32>` is allowed and
  cannot cause a dynamic error; the byte representation is little-endian.
- Any cast from `Bytes<32>` to a foreign field type is allowed and cannot cause
  a dynamic error: the value the byte vector encodes is reduced modulo that
  field's modulus.  A cast from `Bytes<64>` to `Curve25519Scalar` is allowed
  and behaves the same way.
  Round-tripping a foreign field value through `Bytes<32>` therefore always
  gives back the same value, while round-tripping a `Bytes<32>` through a
  foreign field type does so only when the bytes encode an in-range value.
  (Casts from `Bytes` to the native `Field` type behave differently and can
  fail as described above.)
- Apart from these, there are no casts *to* the foreign field types.  Since
  numeric literals have `Uint` type, a literal cannot be used as a foreign
  field value; such a value must come from a witness, a circuit argument, or a
  `Bytes` cast.

There are no casts to or from the curve *point* types.  A point is obtained
from a witness or a circuit argument, or built with `constructJubjubPoint`, and
its coordinates are read with the corresponding accessor circuits.
...
- A JSON info file describing the contract's exports, and a manifest of the
  generated files
...
- `compiler`:
  ...
  - `contract-manifest.json`: a JSON-formatted manifest of every other file the
    compiler generated, with each file's size and SHA-256 sum.
- `contract`:
...
- `zkir`:
  ...
  - One file *circuit-name*.bzkir for each circuit *circuit-name* that requires a proof.
    These files are not generated if the `--skip-zk` flag is supplied when running
    the compiler, or if the compiler cannot find zkir.
- `keys`:
...
- A `declaredInterfaces` table giving, for each contract type the contract calls
  through, the circuit signatures that type declares.
- A `circuitSignatures` table giving the signatures of the contract's own
  circuits.
...
References to `runtime` refer to the `@midnight-ntwrk/compact-runtime` library, which
...
The Compact compiler limits certain datatype values and sizes as follows:
