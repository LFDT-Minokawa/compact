...
This section expands on what the compiler produces and how to interact with them.

### Products of the compiler

The output directory of the compiler has the following subidrectories and products:

- `contract`: this contains the semantics of the contract. It contains an
  `index.js` JavaScript implementation file, an `index.d.ts` type
  declaration file which defines the interface for interacting with the contract,
  an `index.js.map` source-map file which maps the generated JavaScript file to the
  Compact program, and a `contract-info.json` file recording metadata about the compiled
  contract for use by tooling. It captures the compiler, language, and runtime version strings
  used for compiling the contract; a description of every exported circuit including its
  external name, whether it is pure, whether it has an associated proof key, and the names and
  types of its arguments and return value; a description of every witness function with the
  same argument and return type information; and the names of any contract dependencies.
- `zkir`: this contains a zkir circuit generated for each exported impure circuit
  that touches the public ledger
- `keys`: this contains a zero-knowledgge prover and verifier key gegenrated for
  each exported impure circuit that touches the public ledger.
...
- `[`*T*`, ⋯]` — the TypeScript tuple type `[`*S*`, ⋯]` with run-time length checks
  where *S* is the TypeScript representation of the corresponding type *T*
...
- Type aliases — represented as the TypeScript representation of the aliased type.

Note that other `Opaque` types are currently not supported.

### Ledger representation of Compact types
...
- `Uint<0..`*n*`>` - `new CompactTypeUnsignedInteger(`*n*`, `*length*`)`, where
  *length* is the number of bytes required to store *n*-1
- `Uint<`*n*`>` - same as `Uint<0..2<sup>*n*</sup>`>`
- `Bytes<`*n*`>` - `new CompactTypeBytes(`*n*`)`
- `Vector<`*n*`, `*T*`>` - `new CompactTypeVector(`*n*`, `*CT*`)`, where *CT* is the
  `CompactType` for *T*
- `Opaque<"String">` - `CompactTypeOpaqueString`
- `Opaque<"Uint8Array">` - `CompactTypeOpaqueUint8Array`
...
manually or using compiler internals. This also applies to tuple types
that do not have a vector type. However, tuple types that have a vector
type are equivalent to their vector type, i.e., `[`*T*, ⋯, *T*`]` where
all *T*s are the same is equivalent to `Vector<`*n*`, `*T*`>` where *n*
is the number of types in the tuple type and its `CompactType` can be
constructed as `new CompactTypeVector(`*n*`, `*CT*`)`, where *CT* is the
`CompactType` for *T*.

Enumerations are exposed
...
Type aliases are resolved transparently to their underlying type, thus
their `CompactType` are the corresponding `CompactType` of to the aliased type.
