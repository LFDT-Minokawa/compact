...
(library (typescript-passes)
  ...
  ;;TODO from lang weekly and to follow up in next langs weekly:
  ;; - require ledger fields that have contract type to be sealed
  ;; - C.foo.
  ...
  (define-pass print-typescript : Ltypescript (ir) -> Ltypescript ()
    (definitions
      ...
      (define-datatype XPelt
        ...
        (XPelt-external-circuit src internal-id native-entry arg* type external-name)
             ...
             (let ([vminstr* (expand-vm-code
                               ...
                               (map (lambda (path-elt)
                                      (nanopass-case (Ltypescript Path-Element) path-elt
                                        ...
                                        [(,src ,type ,expr) (make-vmref type (Expr expr (precedence add1 comma)))]))
                                    path-elt*)
                               #f
        ...
        (module (print-contract-class)
                                             ...
                                             (ledger-initializers src state pl-array
                                               (set-operations state xpelt0*
                                                 (cons*
                                                   ...
                                                   4 (format "currentPrivateState: ~a.initialPrivateState," constructorContext)
                                                   4 (format "currentZswapLocalState: ~a.initialZswapLocalState," constructorContext)
                                                   4 (format "transactionContext: new __compactRuntime.QueryContext(~a.data, __compactRuntime.dummyContractAddress())" state)
                                                   2 "};"
                                                   2 "const partialProofData = {"
                                                   4 "input: { value: [], alignment: [] },"
                                                   4 "output: undefined,"
                                                   ...
                                                   (ledger-reset-to-default src pl-array
                                                     (list
                                                       2 (Stmt stmt #f)
                                                       2 (format "~a.data = context.transactionContext.state;" state)
                                                       2 "return {"
          ...
          (define (print-unexported-circuit xpelt uname)
            ...
            (XPelt-case xpelt
              ...
              [(XPelt-external-circuit src internal-id native-entry arg* type external-name)
               (print-external-circuit src internal-id native-entry uname arg* type external-name)]
              [else (void)]))

          (define (print-external-circuit src internal-id native-entry uname arg* type external-name)
            (with-local-unique-names
              (let ([q-formal* (map make-Qformal! arg*)]
                    [result (format-internal-binding unique-local-name (make-temp-id src 'result))])
                  ...
                  (let ([q-formal* (map make-Qformal! arg*)])
                    (apply make-Qconcat/src src
                      (make-Qconcat
                        ...
                        ")"
                        0 "{")
                      2 (format "const ~a = __compactRuntime.witnessContext(ledger(context.transactionContext.state), context.currentPrivateState, context.transactionContext.address);" witnessContext)
          ...
          (define (print-contract-query)
            ...
            (display-string "    try {\n")
            (display-string "      res = context.transactionContext.query(prog, __compactRuntime.CostModel.dummyCostModel());\n")
    ...
    (Program-Element : Program-Element (ir export-alist) -> * (xpelt)
      ...
      [(external ,src ,function-name ,native-entry (,arg* ...) ,type)
       (let ([external-name (symbol->string (id-sym function-name))])
         (XPelt-external-circuit src function-name native-entry arg* type external-name))]
    ...
    (Expr : Expression (ir level) -> * (Q)
      ...
      [(or ,src ,[Expr : expr1 (precedence or) -> * expr1] ,[Expr : expr2 (precedence add1 or) -> * expr2])
       (parenthesize level (precedence or)
         (make-Qconcat expr1 0 "||" 0 expr2))]
      [(if ,src ,[Expr : expr0 (precedence add1 if) -> * expr0] ,[Expr : expr1 (precedence if) -> * expr1] ,[Expr : expr2 (precedence if) -> * expr2])
      ...
      [(enum-ref ,src ,type ,elt-name^)
       (parenthesize level (precedence call)
         (nanopass-case (Ltypescript Type) type
           ...
           [else (assert cannot-happen)]))]
      [(tuple ,src ,[Expr : expr* (precedence add1 comma) -> * expr*] ...)
       (make-Qconcat "[" (apply (make-Qsep ",") expr*) "]")]
      [(tuple-ref ,src ,[Expr : expr (precedence vector-ref) -> * expr] ,nat)
       (parenthesize level (precedence vector-ref)
         (make-Qconcat expr (format "[~d]" nat)))]
      [(+ ,src ,mbits ,[Expr : expr1 (precedence add1 comma) -> * expr1] ,[Expr : expr2 (precedence add1 comma) -> * expr2])
       (guard (not mbits))
       (parenthesize level (precedence call)
         (make-Qconcat
      ...
      [(bytes->field ,src ,nat ,[Expr : expr (precedence add1 comma) -> * expr])
       (parenthesize level (precedence call)
         (make-Qconcat
           ...
           ((make-Qsep ",") (format "~d" nat) expr)
           ")"))]
      [(vector->bytes ,src ,nat ,[Expr : expr (precedence add1 comma) -> * expr])
       (parenthesize level (precedence call)
         (make-Qconcat
           "Uint8Array.from("
           ((make-Qsep ",") expr "Number")
           ")"))]
      [(bytes->vector ,src ,nat ,[Expr : expr (precedence add1 comma) -> * expr])
       (parenthesize level (precedence call)
         (make-Qconcat
           "Array.from("
           ((make-Qsep ",") expr "BigInt")
           ")"))]
      [(enum->field ,src ,type ,[Expr : expr (precedence add1 comma) -> * expr])
       (parenthesize level (precedence call)
      ...
      [(downcast-unsigned ,src ,nat ,expr ,safe?)
       (if safe?
           (Expr expr level)
           (let ([expr (Expr expr (precedence add1 comma))])
             (parenthesize level (precedence call)
               (make-Qconcat
                 "((t1) => {"
                 2 (format "if (t1 > ~an)" nat)
      ...
      [(upcast ,src ,type ,type^ ,expr)
       ...
       (Expr expr level)]
      [(public-ledger ,src ,ledger-field-name ,sugar? (,path-elt* ...) ,src^ ,adt-op ,[Expr : expr* (precedence add1 comma) -> * expr*] ...)
       (nanopass-case (Ltypescript ADT-Op) adt-op
         [(,ledger-op ,ledger-op-class (,adt-name (,adt-formal* ,adt-arg*) ...) ((,var-name* ,adt-type*) ...) ,adt-type ,vm-code)
