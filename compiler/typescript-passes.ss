      ...
      [(field->bytes ,src ,nat ,[Expr : expr (precedence add1 comma) outer-pure? -> * expr])
       (parenthesize level (precedence call)
         (make-Qconcat
           ...
           "("
           ((make-Qsep ",") (format "~d" nat) expr (format "'~a'" (format-source-object src)))
           ")"))]
      [(cast-from-bytes ,src ,type ,nat ,[Expr : expr (precedence add1 comma) outer-pure? -> * expr])
       (parenthesize level (precedence call)
         (nanopass-case (Ltypescript Type) type
           [(tfield ,src^)
            (make-Qconcat
           ...
           [(tunsigned ,src^ ,nat^)
            (make-Qconcat
              ...
              "("
              ((make-Qsep ",") (format "~d" nat^) (format "~d" nat) expr (format "'~a'" (format-source-object src)))
              ")")]
           [else (assert cannot-happen)]))]
