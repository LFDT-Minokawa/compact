...
(library (circuit-passes)
  ...
  (define-pass drop-ledger-runtime : Lnodisclose (ir) -> Lposttypescript ()
    ...
    (Expression : Expression (ir) -> Expression ()
      ...
      [(cast-from-bytes ,src ,type ,nat ,[expr])
       (let ([expr `(bytes->field ,src ,nat ,expr)])
         (nanopass-case (Lnodisclose Type) type
           [(tunsigned ,src ,nat) `(downcast-unsigned ,src ,nat ,expr #f)]
           [else expr]))])
    (Type : Type (ir) -> Type ()
    ...
    (Expression : Expression (ir) -> Expression ()
      ...
      [(cast-from-enum ,src ,[type] ,[type^] ,[expr])
       (nanopass-case (Lnoenums Type) type
         [(tfield ,src^) `(upcast ,src ,type ,type^ ,expr)]
         [(tunsigned ,src^ ,nat)
          (let ([maxval (nanopass-case (Lnoenums Type) type^
                          [(tunsigned ,src ,nat) nat]
                          [else (assert cannot-happen)])])
            (cond
              [(> nat maxval) `(upcast ,src ,type ,type^ ,expr)]
              [(< nat maxval) `(downcast-unsigned ,src ,nat ,expr #f)]
              [else expr]))]
         [else (assert cannot-happen)])]
      [(cast-to-enum ,src ,[type] ,[type^] ,[expr])
       (let ([maxval (nanopass-case (Lnoenums Type) type
                       ...
                       [else (assert cannot-happen)])])
         (nanopass-case (Lnoenums Type) type^
           [(tfield ,src^) `(downcast-unsigned ,src ,maxval ,expr #f)]
           [(tunsigned ,src^ ,nat)
            (cond
              [(> nat maxval) `(downcast-unsigned ,src ,maxval ,expr #f)]
              [(< nat maxval) `(upcast ,src ,type ,type^ ,expr)]
              [else expr])]
           [else (assert cannot-happen)]))])
    (Type : Type (ir) -> Type ()
      ...
      (define (maybe-upcast src new-type old-type expr)
        (if (sametype? new-type old-type)
            ...
            (with-output-language (Lunrolled Expression)
              `(upcast ,src ,new-type ,old-type ,expr))))
      )
    (Expression : Expression (ir) -> Expression ()
      (definitions
    ...
    (Care : Expression (ir) -> * (type)
      ...
      [(bytes-slice ,src ,type ,[Care : expr -> * expr-type] ,[Care : index -> * index-type] ,size)
       ...
       (unless (sametype? expr-type type)
         (source-errorf src "expected slice argument to have type ~a, received ~a"
                        type expr-type))
       (let ([nat^ (nanopass-case (Linlined Type) expr-type
                     [(tbytes ,src^ ,nat^) nat^]
      ...
      [(vector-slice ,src ,type ,[Care : expr -> * expr-type] ,[Care : index -> * index-type] ,size)
       ...
       (unless (sametype? expr-type type)
         (source-errorf src "expected slice argument to have type ~a, received ~a"
                        type expr-type))
       (let-values ([(nat^ elt-type) (nanopass-case (Linlined Type) expr-type
                                       ...
                                       [else (source-errorf src "expected slice expr to have a vector type, received ~a"
                                                            (format-type expr-type))])])
         (unless (<= size nat^)
           (source-errorf src "size ~d exceeds the length ~d of the input vector" size nat^))
         (with-output-language (Linlined Type)
      ...
      [(vector->bytes ,src ,nat ,[Care : expr -> * type])
       ...
       (unless (nanopass-case (Linlined Type) type
                 ...
                 [(tvector ,src1 ,nat1 ,type) (and (= nat1 nat) (or (u8-subtype? type) (unknown? type)))]
                 [else #f])
  ...
  (define-pass drop-upcast : Linlined (ir) -> Lnoupcast ()
    (Expression : Expression (ir) -> Expression ()
      [(upcast ,src ,type ,type^ ,[expr]) expr]))

  (define-pass resolve-indices/simplify : Lnoupcast (ir) -> Lnovectorref ()
    (definitions
      (module (no-var-name set-binding! remove-binding! has-binding? get-binding)
    ...
    (Expression : Expression (ir) -> Expression (ctv)
      ...
      [(default ,src ,[adt-type])
       (define (ifdefault-value adt-type k)
         (nanopass-case (Lnovectorref Public-Ledger-ADT-Type) adt-type
           ...
           [(tbytes ,src ,nat) (and (<= nat (field-bytes)) (k (make-bytevector nat 0)))]
           [else #f]))
             ...
             (let f ([adt-type adt-type])
               (or (ifdefault-value adt-type
                   ...
                   (nanopass-case (Lnovectorref Public-Ledger-ADT-Type) adt-type
                     ...
                     [(tvector ,src ,nat ,type) (guard (<= nat 10)) (CTV-tuple no-var-name (make-list nat (f type)))]
      ...
      [(if ,src ,[expr0 ctv0] ,[expr1 ctv1] ,[expr2 ctv2])
      ...
      [(field->bytes ,src ,nat ,[expr ctv])
       (cond
         [(ifconstant ctv
            (lambda (datum)
              (and (or (> nat (field-bytes)) ; quick check when nat is large
                       (> (expt 2 nat) datum))
                   (let ([bv (make-bytevector nat)])
                     (bytevector-uint-set! bv 0 datum (endianness little) nat)
                     bv)))) =>
          (lambda (bv) (values `(quote ,src ,bv) (CTV-const no-var-name bv)))]
         [else (values
                 `(field->bytes ,src ,nat ,expr)
                 (CTV-unknown no-var-name))])]
      [(bytes->field ,src ,nat ,[expr ctv])
       (cond
         [(ifconstant ctv
            (lambda (datum)
      ...
      [(bytes->vector ,src ,nat ,[expr ctv])
       (cond
         ...
         [else (values
                 ...
                 (CTV-unknown no-var-name))])]
      [(downcast-unsigned ,src ,nat ,[expr ctv] ,safe?)
       (cond
      ...
      [(assert ,src ,[Value : expr idset] ,mesg)
       (values
         (if (nanopass-case (Lnovectorref Expression) expr
             ...
             `(assert ,src ,expr ,mesg))
         idset)]
      [(field->bytes ,src ,nat ,[Value : expr idset])
       (values
         `(field->bytes ,src ,nat ,expr)
         idset)]
      [(bytes->field ,src ,nat ,[Value : expr idset])
       (values
         `(bytes->field ,src ,nat ,expr)
         idset)]
      [(vector->bytes ,src ,nat ,[Value : expr idset])
       (values
         `(vector->bytes ,src ,nat ,expr)
         idset)]
      [(bytes->vector ,src ,nat ,[Value : expr idset])
       (values
         `(bytes->vector ,src ,nat ,expr)
         idset)]
      [(downcast-unsigned ,src ,nat ,[Value : expr idset] ,safe?)
       (values
         `(downcast-unsigned ,src ,nat ,expr ,safe?)
         idset)]
              ...
              (let ([path-elt (car path-elt*)] [path-elt* (cdr path-elt*)])
                (nanopass-case (Lnovectorref Path-Element) path-elt
                  ...
                  [(,src ,type ,expr)
                   (Triv expr
                         test
                         (nanopass-case (Lnovectorref Type) type [(tboolean ,src) #t] [else #f])
                         (lambda (triv)
                           (f path-elt*
                              (cons
       ...
       (fold-right
         (lambda (local expr stmt*)
           (nanopass-case (Lnovectorref Local) local
             [(,var-name ,adt-type)
              (Rhs expr test
                (nanopass-case (Lnovectorref Public-Ledger-ADT-Type) adt-type [(tboolean ,src) #t] [else #f])
                (lambda (test rhs)
                  (cons
                    (with-output-language (Lcircuit Statement)
                      `(= ,test ,var-name ,rhs))
  ...
  (define-pass flatten-datatypes : Lcircuit (ir) -> Lflattened ()
    ...
    (Rhs : Rhs (ir test var-name) -> * (stmt*)
      ...
      [(bytes->vector ,src ,nat ,[* wump])
       (let loop ([nat nat] [triv* (reverse (wump->elts wump))] [rvar-name** '()] [stmt* '()])
         (if (fx= nat 0)
      ...
      [(vector->bytes ,src ,nat ,[* wump])
       (let loop ([nat nat] [triv* (wump->elts wump)] [var-name* '()] [stmt* '()])
         (if (fx= nat 0)
             (begin
        ...
        (syntax-rules ()
          [(_ b1 b2 ...)
           (fluid-let ([var->triv (make-eq-hashtable)]
                       ...
                       [fbexpr->vars (make-hashtable nat.triv-hash nat.triv-equal?)]
                       [bvexpr->vars (make-hashtable nat.triv-hash nat.triv-equal?)]
    ...
    (FWD-Multiple : Multiple (ir test var-name* rstmt*) -> * (rstmt*)
      ...
      [(bytes->vector ,src ,[FWD-Triv : triv])
       (with-output-language (Lflattened Statement)
         (or (ifconstant triv
               (lambda (bytes)
                 ; this case is unreachable because resolve-indices/simplify folds bytes->vector calls,
                 ; and we have no way at present to construct a Bytes form that is partly constant and
                 ; partly not.  If we add a syntax for creating Bytes analogous to the syntax for
                 ; creating vectors, we can create such a Bytes.
                 (fold-left
