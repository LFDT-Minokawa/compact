...
(library (passes)
                     ...
                     [else
                      (let* ([lsrc-ir (run-passes parser-passes pathname)]
                        ...
                        (define convert-inner-verifying-key
                          (make-inner-verifying-key-converter))
                        (for-each
  ...
  ;; Returns a pair of an inner verifying key's `verify_proof_vks` entry and its
  ;; lowercase hex SHA-256. The key file is already that entry -- a decider tag
  ;; (`0` for DeciderKind::None, `1` for DeciderKind::Collapsed) followed by the
  ;; processed key, as `midnight_zkir::decider::serialize_vk` writes it -- so the
  ;; decider kind is the key's producer's to declare, not the compiler's.
  ;; Expansion has already checked the tag. Memoized on the resolved pathname:
  ;; one contract may verify against the same key more than once.
  (define (make-inner-verifying-key-converter)
    (let ([table (make-hashtable string-hash string=?)])
      (lambda (vk)
        (let* ([key-pathname (verifying-key-resolved-pathname vk)]
               [a (hashtable-cell table key-pathname #f)])
          (or (cdr a)
              (let ([entry (cons (verifying-key-content vk) (sha256-file key-pathname))])
                (set-cdr! a entry)
                entry))))))
