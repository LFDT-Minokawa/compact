...
(library (analysis-passes)
  ...
  (define-pass expand-modules-and-types : Lpreexpand (ir) -> Lexpanded ()
    (definitions
      ...
      (define-record-type ecdecl-circuit
        (nongenerative)
        (fields elt-name wrapper-name pure? type* type))
                     ...
                     `(tcontract ,src ,contract-name
                        (,(map ecdecl-circuit-elt-name ecdecl-circuit*)
                         ,(map (lambda (ecdecl-circuit)
                                 (let ([fun (lookup-fun p src (ecdecl-circuit-wrapper-name ecdecl-circuit) '())])
            ...
            (if (null? pelt*)
                ...
                (let ([pelt (car pelt*)] [pelt* (cdr pelt*)] [seqno (car seqno*)] [seqno* (cdr seqno*)])
                  ...
                  (nanopass-case (Lpreexpand Program-Element) pelt
                    ...
                    [(external-contract ,src ,exported? ,contract-name (,src* ,pure-dcl* ,elt-name* ,function-name* ((,src** ,var-name** ,type**) ...) ,type*) ...)
                     (let ([info (Info-contract src contract-name (map make-ecdecl-circuit elt-name* function-name* pure-dcl* type** type*) p)])
  ...
  (define-pass identify-pure-circuits : Lnodca (ir) -> Lnodca ()
    ...
    ; NB: we do not process the wrapper circuits that are generated by the compactc
    ; for contract calls.  these wrappers are distinguishable by checking temp-id?
  ...
  (define-pass track-witness-data : Lwithpaths (ir) -> Lwithpaths ()
    ...
    (Expression : Expression (ir p control-witness* disclosing-function-name?) -> * (abs)
      ...
      [(safe-cast ,src ,type (tcontract ,src^ ,contract-name (,elt-name* ,function-name* ,pure-dcl* (,type** ...) ,type*) ...) ,[* abs])
       (Abs-multiple (list abs))]
  ...
  (define-passes fixup-analysis-passes
    (expand-modules-and-types        Lexpanded)
    ;; (generate-contract-ht            Lexpanded)
    ; TODO see if you have test with contract calls for fix up, if not add some.
    (infer-types                     Ltypes))
