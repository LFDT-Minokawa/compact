...
(library (analysis-passes)
           ...
           (values
             ...
             (or (max-type type^*)
                 (source-errorf src "~a should be a vector but has a tuple type ~a that cannot be converted to a vector because its element types are unrelated"
                                what
                                (format-type type))))]
          [(tbytes ,src ,nat) (values nat (with-output-language (Ltypes Type) `(tunsigned ,src 255)))]
    ...
    (Care : Expression (ir) -> Expression (type)
       ...
       (if (memq 'vector-spread kind*)
           ...
           (values
             `(tuple ,src ,(map (lambda (kind nat expr)
                     ...
                     ...)
             (with-output-language (Ltypes Type)
               `(ttuple ,src ,(apply append elt-type**) ...))))]
            ...
            (values
              ...
              (nanopass-case (Ltypes Type) expr-type
                [(ttuple ,src^ ,type* ...)
                 ...
                 (list-ref type* nat)]
                [(tvector ,src^ ,nat^ ,type^)
                 (bounds-check "vector" nat^)
                 type^]
                [else (source-errorf src "expected a tuple, Vector, or Bytes type, received ~a"
                                     (format-type expr-type))])))]
         [else
          (let-values ([(nat^ elt-type) (vector-element-type src "tuple reference with a non-constant index" expr-type)])
            (unless (> nat^ 0)
      ...
      [(tuple-slice ,src ,[Care : expr expr-type] ,[Care : index index-type] ,size)
       ...
       (cond
         [(nanopass-case (Ltypes Type) expr-type
          ...
          (lambda (nat)
            (unless (<= size nat)
              (source-errorf src "slice size ~d exceeds the length ~d of the input Bytes" size nat))
            (values
              ...
              (with-output-language (Ltypes Type)
                (nanopass-case (Ltypes Type) expr-type
                  ...
                  [else (source-errorf src "expected a tuple, Vector, or Bytes type, received ~a"
                                       (format-type expr-type))]))))]
         [else
          (let-values ([(nat^ elt-type) (vector-element-type src "tuple slice with a non-constant index" expr-type)])
            (unless (<= size nat^)
              (source-errorf src "slice size ~d exceeds the length ~d of the input vector" size nat^))
            (let* ([vector-type (with-output-language (Ltypes Type)
                                  `(tvector ,src ,nat^ ,elt-type))]
                   [expr (maybe-upcast src vector-type expr-type expr)])
              (values
      ...
      [(map ,src ,fun ,expr ,expr* ...)
       (let*-values ([(expr+ actual-type+) (maplr2 Care (cons expr expr*))]
                     [(nat actual-elt-type+) (vector-element-types src 'map actual-type+ 2)])
         ...
         (or (and (subtype? type^ type)
             ...
             (T type
                [(tfield ,src1)
                 (T type^
                    [(tbytes ,src2 ,nat2)
                     `(cast-from-bytes ,src ,type ,nat2 ,expr)]
                ...
                [(tbytes ,src1 ,nat1)
                 (T type^
                    ...
                    [(tunsigned ,src2 ,nat2)
                     `(field->bytes ,src ,nat1 (upcast ,src (tfield ,src2) ,type^ ,expr))]
                ...
                [(ttuple ,src1 ,type^* ...)
                 (T type^
                    [(tbytes ,src2 ,nat2)
                     (guard (= nat2 (length type^*)) (andmap u8-supertype? type^*))
                     (maybe-upcast src type
                       (with-output-language (Ltypes Type)
                         `(tvector ,src ,nat2 (tunsigned ,src 255)))
                       `(bytes->vector ,src ,nat2 ,expr))])]
                [(tvector ,src1 ,nat1 ,type^^)
                 (T type^
                    [(tbytes ,src2 ,nat2)
                     (guard (= nat2 nat1) (u8-supertype? type^^))
                     (maybe-upcast src type
                       (with-output-language (Ltypes Type)
                         `(tvector ,src ,nat2 (tunsigned ,src 255)))
                ...
                [(tunsigned ,src1 ,nat1)
                 (T type^
                    [(tfield ,src2)
                     `(downcast-unsigned ,src ,nat1 ,expr #f)]
                    [(tunsigned ,src2 ,nat2)
                     (assert (> nat2 nat1))
                     `(downcast-unsigned ,src ,nat1 ,expr #f)]
                    [(tbytes ,src2 ,nat2)
    ...
    (Tuple-Argument : Tuple-Argument (ir) -> Expression (type kind nat elt-type*)
      ...
      [(spread ,src ,[Care : expr type])
       (nanopass-case (Ltypes Type) type
         ...
         [(tvector ,src ,nat ,type^) (values expr type 'vector-spread nat (list type^))]
         [(tbytes ,src ,nat)
          (let ([expr `(bytes->vector ,src ,nat ,expr)])
            (let* ([type^ (with-output-language (Ltypes Type) `(tunsigned ,src 255))]
                   [type (with-output-language (Ltypes Type) `(tvector ,src ,nat, type^))])
              (values expr type 'vector-spread nat (list type^))))]
    ...
    (Bytes-Argument : Tuple-Argument (ir) -> Tuple-Argument (nat)
      ...
      [(spread ,src ,[Care : expr type])
       (nanopass-case (Ltypes Type) type
         [(tbytes ,src ,nat)
          (values
            `(spread ,src ,nat (bytes->vector ,src ,nat ,expr))
            nat)]
         [(ttuple ,src ,type* ...)
    ...
    (Care : Expression (ir) -> * (type)
      ...
      [(bytes-slice ,src ,type ,[Care : expr -> * expr-type] ,[Care : index -> * index-type] ,size)
       ...
       (unless (sametype? expr-type type)
         (source-errorf src "expected slice argument to have type ~a, received ~a"
                        (format-type type) (format-type expr-type)))
       (let ([nat^ (nanopass-case (Lnodca Type) expr-type
                     [(tbytes ,src^ ,nat^) nat^]
      ...
      [(vector-slice ,src ,type ,[Care : expr -> * expr-type] ,[Care : index -> * index-type] ,size)
       ...
       (unless (sametype? expr-type type)
         (source-errorf src "expected slice argument to have type ~a, received ~a"
                        (format-type type) (format-type expr-type)))
       (let-values ([(nat^ elt-type) (nanopass-case (Lnodca Type) expr-type
                                       [(tvector ,src^ ,nat^ ,type^) (values nat^ type^)]
                                                 ...
                                                 [(spread ,src ,nat ,expr)
                                                  (let ([type (Care expr)])
                                                    (nanopass-case (Lnodca Type) type
                                                      ...
                                                      [(tvector ,src ,nat ,type) (values nat (list type))]
                                                      [(tbytes ,src ,nat) (values nat (list `(tunsigned ,src 255)))]
      ...
      [(cast-to-enum ,src ,type ,type^ ,[Care : expr -> * type^^])
       (unless (sametype? type^^ type^)
         (source-errorf src "expected ~a, got ~a for cast-to-enum"
                        ...
                        (format-type type^^)))
       type]
      [(cast-from-bytes ,src ,[type] ,nat ,[Care : expr -> * type^])
       (nanopass-case (Lnodca Type) type^
         [(tbytes ,src ,nat^)
          (unless (= nat^ nat)
      ...
      [(vector->bytes ,src ,nat ,[Care : expr -> * type])
       (define (u8-subtype? type)
         (nanopass-case (Lnodca Type) type
           ...
           [else #f]))
       (unless (nanopass-case (Lnodca Type) type
                 [(ttuple ,src1 ,type* ...) (and (= (length type*) nat) (andmap u8-subtype? type*))]
                 [(tvector ,src1 ,nat1 ,type) (and (= nat1 nat) (u8-subtype? type))]
                 [else #f])
  ...
  (define-pass track-witness-data : Lwithpaths (ir) -> Lwithpaths ()
    ...
    (Expression : Expression (ir p control-witness* disclosing-function-name?) -> * (abs)
      ...
      [(cast-from-bytes ,src ,type ,nat ,[* abs]) abs]
      [(field->bytes ,src ,nat ,[* abs]) abs]
      [(bytes->vector ,src ,nat ,[* abs]) (Abs-single (Abs-atomic (abs->witnesses abs)))]
      [(vector->bytes ,src ,nat ,[* abs]) (Abs-atomic (abs->witnesses abs))]
