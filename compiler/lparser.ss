...
(library (lparser)
  ...
  (define-language/pretty Lparser
    ...
    (Statement (stmt)
      ...
      (const src kwd parg op expr semicolon) => (const parg #f expr)
    ...
    (Expression (expr)
      ...
      (tuple src lbracket (expr* ...) (comma* ...) rbracket) => (tuple expr* ...)
      (tuple-ref src expr lbracket nat rbracket) => (tuple-ref #f expr #f nat)
