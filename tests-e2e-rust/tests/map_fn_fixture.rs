...
//
// map_fn_fixture.compact byte-parity test — validates Iter 7 of the
// rust-codegen polish work (map() + lambda-as-expression).
//
// Drives the generated map-fn-fixture crate through initial_state()
// and asserts the serialized ContractState matches the TS reference
// fixture captured by fixtures/capture-map-fn-fixture.mjs.
//
// Purpose: validate the `(map fn iterable)` IR lowering. The Compact
// source constructor seeds a `Vector<3, Uint<64>>` ledger field by
// applying an identity lambda over a literal `[1u64, 2, 3]` array;
// the emitter must produce a `let tmp = [1u64, 2, 3];` binding and a
// `new_cell_array(tmp.clone())` Cell.write so the on-chain state
// matches the TS-side semantics (one Cell holding three concatenated
// 8-byte little-endian atoms).
//
// The `ping` circuit is a placeholder `c = map((x) => x, [0, 0, 0])`
// that forces impure classification — it's not exercised by the
// init-state assertion below, but its emission goes through the same
...
fn fixture() -> SmallFixtureTsReference {
    SmallFixtureTsReference::load(concat!(
        ...
        "/fixtures/map-fn-fixture-ts-state.json"
    ))
}
...
/// map_fn_fixture exports a single circuit `ping`, so the operations
...
#[test]
fn map_fn_fixture_init_byte_parity() {
    let ts_ref = fixture();
