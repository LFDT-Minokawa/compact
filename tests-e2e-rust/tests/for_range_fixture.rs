...
//
// for_range_fixture.compact byte-parity test — validates Iter 4 of the
// rust-codegen polish work.
//
// Drives the generated for-range-fixture crate through initial_state()
// and asserts the serialized ContractState matches the TS reference
// fixture captured by fixtures/capture-for-range-fixture.mjs.
//
// Purpose: validate the for-range loop emission MVP. The Compact source
// constructor seeds a Counter ledger field to 10 via a literal-bounds
// `for (const i of 0..10) { c.increment(1); }` loop; the emitter
// compile-time unrolls the body's builder lines (.idx_at_index/.addi/
// .ins) ten times into a single OpProgramVerify chain, which mirrors
...
fn fixture() -> SmallFixtureTsReference {
    SmallFixtureTsReference::load(concat!(
        ...
        "/fixtures/for-range-fixture-ts-state.json"
    ))
}
...
/// for_range_fixture exports a single circuit `ping`, so the operations
...
#[test]
fn for_range_fixture_init_byte_parity() {
    let ts_ref = fixture();
