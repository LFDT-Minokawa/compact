...
// -------------------------------------------------------------------------
// Merkle path root — stdlib circuits routed via `stdlib-circuit-rust-path`.
// -------------------------------------------------------------------------

/// Compact's `merkleTreePathRoot<#n, T>(path: MerkleTreePath<n, T>): MerkleTreeDigest`
/// — computes the Merkle root reachable from a path. Delegates to upstream
/// `MerklePath::root()` (see midnight-transient-crypto/src/merkle_tree.rs:201).
/// The const-generic `n` (path height) is captured by the `MerklePath<T>` type
/// via its `path: Vec<MerklePathEntry>`, so the wrapper only needs `T`.
///
/// Note: generated contract code currently emits a per-contract `MerkleTreePath`
/// struct (leaf + fixed-N array of entries) rather than this upstream type, so
/// the wrapper isn't directly callable from contracts today. The routing is in
/// place for when the codegen migrates to use this upstream-shaped path
/// directly (or via a conversion shim).
...
/// Compact's `merkleTreePathRootNoLeafHash<#n>(path: MerkleTreePath<n, Bytes<32>>):
/// MerkleTreeDigest` — like `merkle_tree_path_root` but skips the leaf-hash
/// step (the leaf is already a 32-byte digest, so we just `degradeToTransient`
/// it before folding).
///
/// Upstream `MerklePath::root()` unconditionally applies `leaf_hash` to the
/// leaf, so this variant cannot delegate to `.root()` directly when the
/// leaf is `[u8; 32]`. The body here mirrors the stdlib source: degrade the
/// raw 32-byte leaf, then fold the path entries with the same combiner.
