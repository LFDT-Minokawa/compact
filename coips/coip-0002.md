...
An `contract` declaration introduces a new contract type into its
...
2. If any cycles are present in the call graph of cross-contract
   ...
   application.
3. A contract that declares witnesses is unable to satisfy any
   contract type.  This implies that every contract reference is to a
...
from satisfying a contract type.  A called circuit, *as expressed in
