...
3. Circuits called from other contracts must not rely on witness
   ...
   circuits that do not call witness functions.
...
declarations for all the type's declared circuits, no matter
...
fails to export the circuits required by the contract type.
...
3. Any circuit that calls witness functions is excluded from
   consideration for satisfying a contract type.  This implies that
   every contract reference is to a contract that does not manipulate
   private state in the circuits that can be called through the
   reference.  The called contract may define witness functions, but
   those functions cannot be called directly or indirectly by the
   exported circuits that satisfy a contract type.
...
witness functions.  Furthermore, the concrete implementations of all
contracts in the system are non-recursive and known to any DApp
...
rely on those witnesses, while other do not.  The full set of circuits
...
multi-contract systems with private state.  Whie awaiting such a
