...
test('Check fib 0', () => {
  var [c, Ctxt] =  startContract(contractCode, witnessSets, 0);
...
test('check private state', () => {
  var  [c, Ctxt] = startContract(contractCode, witnessSets, 1);
  Ctxt = c.circuits.fib2(Ctxt, false).context;
...
test('check contract address', () => {
  var  [c, Ctxt] = startContract(contractCode, witnessSets, 0);
  Ctxt = c.circuits.fib2(Ctxt, false).context;
  expect(Ctxt.currentQueryContext.address).toEqual(Ctxt.contractAddress)
});
