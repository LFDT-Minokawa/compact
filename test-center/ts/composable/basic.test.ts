...
import * as InnerModule from '../../composable/Basic/Inner/contract/index.js';
import * as OuterModule from '../../composable/Basic/Outer/contract/index.js';
import {
  deployDependency,
  startContractGroup,
  flushProofChecks
...
const InnerCode = {
  ...InnerModule,
  contractDir: 'composable/Basic/Inner',
};
const OuterCode = {
  ...OuterModule,
  contractDir: 'composable/Basic/Outer',
};

/** Read the `v` field of the Inner ledger from a cached QueryContext. */
const innerV = (
  ctxt: CircuitContext<unknown>,
  ...
  return InnerModule.ledger(qc.state).v;
};
...
describe('setInner swaps which Inner is current', () => {
  ...
  test('outer.inner ledger field reflects the most recent setInner', async () => {
    ...
    const outerLedger0 = OuterModule.ledger(
    ...
    const outerLedger1 = OuterModule.ledger(
