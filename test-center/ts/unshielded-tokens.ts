...
test('mintUnshieldedToSelfTest', () => {
  ...
  let [c, context] = startContract(contract, 0);
  ...
  expectEmptyEffectsExcept(context.transactionContext.effects, ['claimedUnshieldedSpends', 'unshieldedMints']);

  const claimedUnshieldedSpends = context.transactionContext.effects.claimedUnshieldedSpends;
  const unshieldedMints = context.transactionContext.effects.unshieldedMints;

  const rawSelfAddress = context.transactionContext.address;
...
test('sendUnshieldedToSelfTest', () => {
  ...
  context = c.circuits.sendUnshieldedToSelfTest(context, unshieldedCoinInfo).context;

  expectEmptyEffectsExcept(context.transactionContext.effects, ['claimedUnshieldedSpends', 'unshieldedOutputs']);

  const claimedUnshieldedSpends = context.transactionContext.effects.claimedUnshieldedSpends;
  const unshieldedOutputs = context.transactionContext.effects.unshieldedOutputs;
...
test('sendUnshieldedToContractTest', () => {
  ...
  context = c.circuits.sendUnshieldedToContractTest(context, unshieldedCoinInfo, recipient).context;
...
test('sendUnshieldedToUserTest', () => {
  ...
  context = c.circuits.sendUnshieldedToUserTest(context, unshieldedCoinInfo, recipient).context;
...
test('receiveUnshieldedTest', () => {
  ...
  context = c.circuits.receiveUnshieldedTest(context, unshieldedCoinInfo).context;

  expectEmptyEffectsExcept(context.transactionContext.effects, ['unshieldedInputs']);

  const unshieldedInputs = context.transactionContext.effects.unshieldedInputs;
...
test('getUnshieldedBalanceTest', () => {
  ...
  context.transactionContext.block = {
    ownAddress: context.transactionContext.address,
