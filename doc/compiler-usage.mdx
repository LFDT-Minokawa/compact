...
- a contract-information file _targetdir_**/compiler/contract-info.json**,

- one Zk/ir circuit file _targetdir_**/zkir/**_circuitname_**.zkir** and
  one binary Zk/ir circuit file _targetdir_**/zkir/**_circuitname_**.bzkir**
  for each circuit _circuitname_ that requires a proof,

- a pair of proving keys _targetdir_**/keys/**_circuitname_**.prover** and
  _targetdir_**/keys/**_circuitname_**.verifier** for each circuit _circuitname_
  that requires a proof,
  and

- a manifest file _targetdir_**/compiler/contract-manifest.json**, organized
  by directory, listing the generated files identified above along with their
  sizes and SHA-256 sums.
...
Prints the compiler version and exits.  Builds that recorded a commit follow
the version with that commit and its date in parentheses.

**--verbose**

Given together with **--version**, in either order, prints the version, commit,
commit date, and language and runtime versions as separate fields, showing
_unknown_ for anything the build did not record.
...
Causes the compiler to generate circuits using ZKIR version 3,
...
obj/test/compiler/contract-info.json
obj/test/compiler/contract-manifest.json

obj/test/zkir/foo.zkir
obj/test/zkir/foo.bzkir
...
**compactc** exits with status 0 on success and with a nonzero status on
failure.  In particular, it exits with status 1 if the command line is
malformed.
