...
The compiler recognizes that certain Compact standard library routines sufficiently disguise witness data so that explicit declaration of disclosure is not required. For example, the commitment functions `transientCommit` and `persistentCommit` are both recognized as such on the assumption that their `rand` argument is sufficiently random.
On the other hand, the hash functions `transientHash`, `persistentHash`, `keccak256` and `sha512` are not, i.e., their results are treated as containing the same witness data of their input, although the compiler reports failure to declare disclosure of the result of one of these functions as the exposure of a hash of the witness data rather than of the witness data itself.
