...
Compact is a statically typed, bounded smart contract language,
...
- definitions of `circuits`, which are functions serving as the operational core of a smart contract.
...
Like both TypeScript and JavaScript, Compact is an eager call-by-value language.

Compact deviates intentionally from TypeScript, however, in several important ways:
- Unlike TypeScript, it is not possible to work around the static type system via
  missing type declarations, unsafe casts.
...
- Because every Compact program must compile into a set of finite proving circuits,
  ...
  is disallowed.
- Compact supports only unsigned integers either with a program-declared range or
  with a range that is determinded by the field size of the target proving system.
...
It does so rather than simply producing TypeScript for three reasons:
...
- so that it can generate a sourcemap file that properly maps elements of the
  generated JavaScript code (e.g., variable bindings) to the corresponding
  elements of the source Compact code.

For each circuit that touches the public ledger and hence requires a proof for
on-chain executation, the Compact compiler also produces proving circuits in a
...
The language is **strongly statically typed**: the compiler rejects programs that do
not type check.  For example, it rejects programs in which a circuit or witness with a
parameter type annotation is called with an incorrectly typed argument for that
parameter, Iand it rejects programs where a circuit with a return-type
