...
- Indentity casts, i.e., casts from a type to itself, are always allowed (but
  never required), never result in run-time errors, and never require run-time
  conversions.
- Upcasts, i.e., casts from a type to a supertype that is not the same type
  (e.g., from `Vector<3, Uint<8>>` to `Vector<3, Uint<16>>`, are always allowed
  (but never required), never result in run-time errors, and might require run-time
  conversions.
- Downcasts, i.e., casts from a type to a subtype that is not the same type,
  are always allowed, always require run-time checks, and might require run-time
  conversions.
...
!!!!!!!! Might want to mention impliciations of these rules, such as rules for casting tuples to vectors and back
