...
Each parameter is either a type name (e.g., `T`) or a hash-prefixed numeric name
(e.g., `#N`).
Nat names are prefixed by `#` to distingish generic numeric parameters from
...
or numeric parameter that is visible at the point of specialization.

The `#` used to distinguish generic numeric parameters from generic type parameters
...
- `Uint<m..n>`, where `m` is the literal `0` or generic numeric parameter bound
  to `0`, and where `n` is a natural number literal or a generic
  numeric parameter, is the type of *bounded unsigned integer* values between
...
- `Uint<n>`, where `n` is a non-zero natural number literal or generic numeric
...
- `Vector<n, T>`, where `n` is a natural number literal or generic numeric
...
- `Bytes<n>`, where `n` is a natural number literal or a generic numeric parameter,
...
!!!!!!!! TBD Kent: need to say that patterns give rise to variable bindings and what types those bindings have
...
or a generic numeric parameter, and `end` must be greater than or
equal to `start`.
...
generic module or circuit in which a reference to a generic numeric parameter
...
subexpression.  It is a static error if this type is not `Boolean`.  The type of
...
an expression and `index` is a numeric literal, generic numeric parameter reference,
...
- Any `index` can be a constant, i.e., a numeric literal or a generic numeric
  parameter reference.
...
- If the type of `expr` a vector or byte-vector type, `index` must be a
  *constant-valued* expression, where a constant-valued expression is:
  - a constant,
  - a reference to a generic numeric parameter,
...
`Ti` if `index` is the constant `i` or the value `i` of a generic numeric
parameter.
...
an expression, and `index` is a numeric literal, a generic numeric
...
subexpression.  It is a static error if this type is not a structure type.  It is
a static error if the structure type does not contain a member with the same name
...
* **For an anonymous circuit:** Parameters that have type annotations will be
  ...
  The type of each variable binding arising from the binding of identifiers in
  each parameter pattern to the corresponding pieces of the corresponding argument
  is the type of the corresonding part of the structure of the declared type,
  if present, otherwise of the inferred type.
  ...
  types.  It is a static error if there is a returned subexpression whose
...
any other use would be a static error.
