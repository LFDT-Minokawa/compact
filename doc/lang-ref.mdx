...
A module can have [generic parameters](#generic-parameters), in which case it is
...
<table className="lang-ref-table"><tbody><tr><td><em>constructor-definition</em>&nbsp;</td><td>&rarr;</td><td>&nbsp;&nbsp;<b><tt>constructor</tt></b>&nbsp;&nbsp;<a href="#Pattern-parameter-list"><em>pattern-parameter-list</em></a>&nbsp;&nbsp;<a href="#Block"><em>block</em></a></td></tr><tr><td><em>pattern-parameter-list</em>&nbsp;</td><td>&rarr;</td><td>&nbsp;&nbsp;<b><tt>(</tt></b>&nbsp;&nbsp;<a href="#Typed-pattern"><em>typed-pattern</em></a> <b><tt>,</tt></b> &mldr; <b><tt>,</tt></b> <a href="#Typed-pattern"><em>typed-pattern</em></a> <b><tt>,</tt></b><sup>opt</sup>&nbsp;&nbsp;<b><tt>)</tt></b></td></tr> </tbody></table>
...
The effects of casting an expression `e` of type `T` to another type `U`
depend on the specific types involved.
For some types `T` and `U`, the cast is not allowed, which is a static error.
Even if the cast is allowed, a run-time error might occur if the value of `e`
cannot be represented as a value of type `U`.
In some cases, the run-time representation of `T` is different from the run-time
representation of `U`, in which case the value of `e` must be converted at
run time from the one representation to the other.

The following rules govern when casts are allowed, when a cast might cause a
run-time error, and when a cast require a run-time conversion.

!!!!!!!! TBD verify all of these claims

- Casts from a type to the same type are always allowed (but never required),
  never result in run-time errors, and never require run-time conversions.
- Upcasts, i.e., casts from a type to a supertype (e.g., from `Vector<3, Uint<8>>`
  to `Vector<3, Uint<16>>`, are always allowed (but never required) and never
  result in run-time errors.
  They might require run-time conversions.
- Downcasts, i.e., casts from a type to a subtype, are always allowed, always
  require run-time checks, and might require run-time conversions.
  For example, a value of type `Vector<3, Uint<16>>` can be cast to type
  `Vector<3, Uint<8>>` with a run-time check of each element and possibly
  a conversion if the representations differ.
- Casts from `Boolean` to `Field` or `Uint<0..m>`, `m > 1`, are always allowed,
  do not require a run-time check, but require a simple run-time conversion (of
  `false` to `0` and `true` to `1`).
  Casts from `Boolean` to `Uint<0..1>` require a run-time check, i.e., that the
  value is `false`.
- Casts from `Field` or `Uint<0..m>`, for all `m`, to Booleans are always allowed,
  cannot result in a run-time error, and require a simple run-time conversion
  (of `0` to `false` and any other value to `true`).
...
!!!!!!!! Might want to mention impliciations, such as rules for casting tuples to vectors and back

No other casts are allowed.

!!!!!!!! TBD flush this table, decide if any of the notes are still useful
