...
The order of program elements in a module is unimportant.
...
Exporting a module binding by including the `export` modifier in the module
definition has the effect of exporting the module binding from the enclosing module.
It is a static error if the `export` modifier appears on module definition at
the program's top level, i.e., not enclosed within the body of another module
definition.
