...
an explicit return value, i.e., the value of `expression` in this form of `return`:
...
Otherwise, a `return` statement is well-typed if the type of `expression`, or
`[]` if no `expression` is present, is a subtype the expected return type.
...
When a `return` statement is evaluated, the `expression`, if present, is evaluated
first, the circuit or constructor returns immediately without evaluating any
subsequent statements, and it returns to the caller the value of `expression` or
