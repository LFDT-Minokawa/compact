...
- Upcasts, i.e., casts from a type to a supertype (e.g., from `Vector<3, Uint<8>>`
  to `Vector<3, Uint<16>>`, are always allowed (but never required), never
  result in run-time errors, and might require run-time conversions, though
  never when the type and supertype are the same.
...
The type of any cast `e as T` is, naturally, `T`.
