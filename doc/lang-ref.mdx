...
generic module or circuit in which a reference to a generic size parameter
...
subexpression.  It is a static error if this type is not a tuple type `[T, ...]`.
It is a static error if the length of the tuple is less than or equal to the
