...
separately. This document explains each syntactic category individually.  It
starts by introducing the basics that are used in various syntactic categories:
identifiers, constants, types, generic parameters, and patterns and argument.
Then it expands on programs and program elements and opens up each of the
program elements separately.  Where applicable it expands a syntactic category
from lowest precedence to highest.  Finally, it discusses the TypeScript target.
...
- Repetition is specified by ellipses. The
  ...
  grammar symbol and <b><tt>,</tt></b> is a separator, represents zero or more
  occurrences of <em>X</em> separated by <b><tt>,</tt></b>. In either case, when
  ...
  by <em>,</em><sup>opt</sup>, an optional trailing separator is allowed, but
  ...
  <em>expr</em>s possibly followed by an extra comma. There are two separators:
  comma and semicolon.
