...
- Upcasts, i.e., casts from a type to a supertype (e.g., from `Vector<3, Uint<8>>`
  to `Vector<3, Uint<16>>`, are always allowed (but never required), never result
  in run-time errors, and, except in the case where the type and the supertype are
  the same, might require run-time conversions.
