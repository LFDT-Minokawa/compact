...
- `Uint<m..n>`, where `m` is the literal `0` or generic natural-number parameter bound
  to `0`, and where `n` is a positive natural number literal or a generic
  natural-number parameter, is the type of *bounded unsigned integer* values between
...
given in [Compiler Usage](reference/tools/compiler-usage.md).
...
See [here](/docs/develop/reference/midnight-api/compact-runtime/README.md)

### Compiler usage

See [here](reference/tools/compiler-usage).
