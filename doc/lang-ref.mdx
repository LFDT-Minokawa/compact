...
A statement may be
- a [expression sequence](#expression-sequence)
- a [`const` binding](#const-binding-statement)
- an [`if` statement](#if-statement)
- a [`for` loop](#for-loop)
- a [`return` statement](#return-statement)
- a block - a sequence of zero or more statements in a nested scope, enclosed by curly braces
