...
- `Uint<m..n>`, where `m` is the literal `0` or a generic size parameter in
  ...
  although [one may be a subtype of the other](./subtyping#subtyping-and-least-upper-bounds).  In practice, there
...
- `[T, ...]`, where `T, ...` are zero or more comma-separated types is the type
  ...
  of the other](./subtyping#subtyping-and-least-upper-bounds).
