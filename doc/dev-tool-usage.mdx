...
Sets the Compact artifact directory.
...
Prints a version and exits.  For **compile** the flag is forwarded to the
installed compiler, so it prints the compiler version; for **format** and
**fixup** it prints the toolchain version.
...
forwards the flag to the installed compiler and returns **compactc**'s own help
text.  With no compiler installed, this reports an error rather than printing
help.
...
Print the toolchain version, or the language version supported by it, and exit.

Note that **compact format** does not forward flags to the underlying
format-compact tool: only the file paths are passed through, so flags such as
**--line-length** and **--vscode** are rejected here.  To use them, invoke
format-compact directly; visit [the format usage page](formatter-usage.mdx) for
a more detailed explanation of that tool.
...
Adjusts the end point of each `Uint` whose size is given by a range with a
constant end point, and warns for each `Uint` whose range end point is a
generic-variable reference.  Forwarded to the underlying fixup-compact tool.

**--vscode**

Causes error messages to be printed on a single line, for the VS Code
extension.  Forwarded to the underlying fixup-compact tool.

**--verbose**

Prints each file processed along with its outcome.  Without this flag only
...
Only **--update-Uint-ranges** and **--vscode** are forwarded to the underlying
fixup-compact tool; flags such as **--compact-path**, **--trace-search** and
**--line-length** cannot be passed through this command.  To use them, invoke
fixup-compact directly; visit [the fixup usage page](fixup-usage.mdx) for a more
detailed explanation of that tool.
...
set. When **--cache** is set, it also removes the cache file. When
...
removes the cache file (`github_cache.json`) as well.
